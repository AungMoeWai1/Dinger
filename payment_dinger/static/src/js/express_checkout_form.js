...
import { StripeOptions } from '@payment_stripe/js/dinger_options';
...
paymentExpressCheckoutForm.include({
    ...
    async _prepareExpressCheckoutForm(providerData) {
         ...
         */
        if (providerData.providerCode !== 'dinger' || !this.paymentContext['amount']) {
            this._super(...arguments);
            return;
        }

        const dingerJS = Dinger(
            providerData.stripePublishableKey,
            new DingerOptions()._prepareDingerOptions(providerData),
        );
        const paymentRequest = dingerJS.paymentRequest({
        ...
        if (this.dingerPaymentRequests === undefined) {
            this.dingerPaymentRequests = [];
        }
        this.dingerPaymentRequests.push(paymentRequest);
        ...
        paymentRequest.on('paymentmethod', async (ev) => {
            ...
            const { paymentIntent, error: confirmError } = await dingerJS.confirmCardPayment(
    ...
    _updateAmount(newAmount, newMinorAmount) {
        this._super(...arguments);
        this.dingerPaymentRequests && this.dingerPaymentRequests.map(
